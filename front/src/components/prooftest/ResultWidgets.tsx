...
import { getProofTestResultTypeLabel } from './proofTestI18n'
...
export function ResultBadge({ r }: { r: StepResultValue }) {
  ...
  if (r === 'oui') return <span className="flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded border" style={{ color: semantic.success, background: `${semantic.success}12`, borderColor: `${semantic.success}30` }}><CheckCircle2 size={9} />{strings.widgets.resultBadge.yes}</span>
  if (r === 'non') return <span className="flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded border" style={{ color: semantic.error, background: `${semantic.error}10`, borderColor: `${semantic.error}28` }}><XCircle size={9} />{strings.widgets.resultBadge.no}</span>
  if (r === 'na') return <span className="flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded border" style={{ color: TEXT_DIM, background: PAGE_BG, borderColor: BORDER }}><Minus size={9} />{strings.widgets.resultBadge.na}</span>
...
export function ResultInput({ step, result, disabled = false, onChange }: {
  ...
  if (step.resultType === 'oui_non') {
    const labelByValue: Record<Exclude<StepResultValue, null>, string> = {
      oui: strings.widgets.resultBadge.yes,
      non: strings.widgets.resultBadge.no,
      na: strings.widgets.resultBadge.na,
    }
    return (
      <div className="flex items-center gap-1">
        {(['oui', 'non', 'na'] as Exclude<StepResultValue, null>[]).map(v => (
          <button key={v}
            disabled={disabled}
    ...
    return (
      <div className="flex items-center gap-1.5">
        <input
          ...
          placeholder={strings.widgets.resultInput.measuredValuePlaceholder}
          className={cn(
  ...
  return (
    <input
      ...
      placeholder={strings.widgets.resultInput.customResultPlaceholder}
  ...
  if (step.resultType === 'oui_non') {
    return (
      <span className="flex items-center gap-1">
        <span className="text-[9px] font-bold px-1 rounded border" style={{ color: semantic.success, background: `${semantic.success}12`, borderColor: `${semantic.success}30` }}>{strings.widgets.resultBadge.yes}</span>
        <span className="text-[9px]" style={{ color: TEXT_DIM }}>{strings.widgets.expectedValue.or}</span>
        <span className="text-[9px] font-bold px-1 rounded border" style={{ color: semantic.error, background: `${semantic.error}10`, borderColor: `${semantic.error}28` }}>{strings.widgets.resultBadge.no}</span>
      </span>
    )
  }
  return (
    <span className="font-mono text-[11px] font-semibold" style={{ color: TEXT }}>
      ...
      {step.expectedValue && step.resultType !== 'valeur' && (
        <span className="sr-only">{getProofTestResultTypeLabel(strings, step.resultType)}</span>
      )}
    </span>
  )
}
