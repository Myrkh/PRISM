    ...
    <div className="flex h-full flex-col overflow-hidden" style={{ background: PANEL_BG }}>
      <div className="flex-1 overflow-y-auto" style={{ scrollbarGutter: 'stable' }}>
        <div className="sticky top-0 z-10 px-3 pt-3" style={{ background: PANEL_BG }}>
          <IntercalaireTabBar
            tabs={OVERVIEW_RIGHT_TABS}
            active={activeTab}
            onSelect={tab => setActiveTab(tab as (typeof OVERVIEW_RIGHT_TABS)[number]['id'])}
            cardBg={CARD_BG}
          />
        </div>

        <div className="px-3 pb-3">
          <IntercalaireCard tabCount={OVERVIEW_RIGHT_TABS.length} activeIdx={activeIndex} className="space-y-3 p-3">
            {activeTab === 'snapshot' && (
              <>
          ...
          <div className="mt-3 rounded-xl border p-3" style={{ borderColor: BORDER, background: PAGE_BG }}>
