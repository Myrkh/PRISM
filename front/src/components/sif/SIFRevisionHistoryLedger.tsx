...
function InlineDelta({ older, newer }: { older: SIFRevision; newer: SIFRevision }) {
  ...
  if (Math.abs(calcOld.PFD_avg - calcNew.PFD_avg) > 1e-12) {
    parts.push(`PFD ${formatPFD(calcOld.PFD_avg)} → ${formatPFD(calcNew.PFD_avg)}`)
  }
...
export function SIFRevisionHistoryLedger({ sif }: { sif: SIF }) {
  const { BORDER, PAGE_BG, SURFACE, TEAL, TEAL_DIM, TEXT, TEXT_DIM, semantic } = usePrismTheme()
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 rounded-xl border px-3 py-3" style={{ borderColor: BORDER, background: PAGE_BG }}>
          <div className="min-w-0">
            <p className="text-xs font-semibold" style={{ color: TEXT }}>
              {sif.revisionLockedAt ? `Révision ${sif.revision} publiée` : `Révision ${sif.revision} en travail`}
            </p>
            <p className="mt-1 text-[11px] leading-relaxed" style={{ color: TEXT_DIM }}>
              {sif.revisionLockedAt
                ? `Dernière publication le ${new Date(sif.revisionLockedAt).toLocaleDateString()}.`
                : 'Les lignes ci-dessous correspondent aux publications figées précédentes; la révision courante reste modifiable.'}
            </p>
          </div>
          ...
          <div className="grid min-w-[220px] gap-2 sm:grid-cols-2">
            <div className="rounded-lg border px-3 py-2" style={{ borderColor: BORDER, background: SURFACE }}>
              <p className="text-[9px] font-bold uppercase tracking-widest" style={{ color: TEXT_DIM }}>Snapshots</p>
              <p className="mt-1 text-sm font-semibold font-mono" style={{ color: TEAL_DIM }}>{publishedRevisions.length}</p>
            </div>
            <div className="rounded-lg border px-3 py-2" style={{ borderColor: BORDER, background: SURFACE }}>
              <p className="text-[9px] font-bold uppercase tracking-widest" style={{ color: TEXT_DIM }}>Dernière publication</p>
              <p className="mt-1 text-sm font-semibold" style={{ color: lastPublishedAt ? TEXT : TEXT_DIM }}>
                {lastPublishedAt ? new Date(lastPublishedAt).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }) : 'Aucune'}
              </p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full min-w-[960px] table-fixed border-collapse">
              ...
              <thead>
                <tr className="border-b" style={{ borderColor: BORDER, background: PAGE_BG }}>
                  {(['Rév.', 'Statut', 'Date', 'Établi par', 'Objet', 'Artefacts & compare'] as const).map(label => (
                    <th
                      key={label}
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-4 py-4 text-[11px] italic" style={{ color: TEXT_DIM }}>
                      Aucune révision publiée pour cette SIF. Clôture une première révision pour figer le dossier et ses artefacts.
                    </td>
                  </tr>
                ) : (
                    ...
                    return [
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 align-top whitespace-nowrap text-[11px]" style={{ color: TEXT_DIM }}>
                          {new Date(revision.createdAt).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}
                        </td>
